...
for file in $path/data_*400*cat*.parquet
do
    newfile=${file%'_cat'*}
    i=${newfile##*'data_'}
    cat=${file%.*}
    ...
    python3 makeDatacard.py --years 2018,2017,2016pre,2016post --prune --ext sys_resolved_combine_M1000_"$cat"_M$i  --output Datacard_sys_resolved_combine_M1000_"$cat"_M$i 
done
